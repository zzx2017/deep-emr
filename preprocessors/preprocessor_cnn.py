...
def generate_files(data, labels, files):
    for i in range(0, len(data)):
        file = open('../models/cnn/data/training/' + files[i][17:-4] + '.txt', 'w')
        for j in range(0, len(data[i])):
            classes = list(set([x for x in labels[i][j] if x != 1]))
            if len(classes) == 0:
                classes = [0]
            file.write(','.join(str(x) for x in data[i][j]) + ' ' + (','.join(str(x) for x in classes)) + '\n')
        file.close()
